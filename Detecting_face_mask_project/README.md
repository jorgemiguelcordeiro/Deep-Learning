...
The dataset was sourced from Kaggle ([link](https://www.kaggle.com/datasets/jamesnogra/face-mask-usage)) and collected using OpenCV. The images come from YouTube videos and mobile recordings. The model is trained to recognize masks under different conditions, including occlusion, deformations, and various lighting environments.  

---

# **3 - Evaluation Measures**  
We use standard metrics for multi-class classification problems, including **Accuracy, Recall, and F1-Score**. Additionally, we focus on the **Precision of the Fully Covered class**, as false positives in this category represent the worst type of misclassification. To avoid unnecessary computation and overfitting, an **Early Stopping** callback is implemented, halting training if validation accuracy does not improve for two consecutive epochs.  

Our initial strategy involves training models using the **Hold-out Method** to identify promising parameters in terms of accuracy, precision, and training time. Once we identify the best configurations, we apply **Cross-Validation** to obtain more realistic metric values for the selected models.  

---

# **4 - Hold-out Method**  
Our first approach was the **Hold-out Method**, where we sequentially split the dataset into:  

- **80% for training**  
- **10% for validation**  
- **10% for testing**  

To automate this process, we implemented Python functions that create structured folders for train, validation, and test sets, with subfolders corresponding to each of the four classes. Images were rescaled from **0-255 to 0-1** for improved model efficiency.  

We also created **data generators** to yield batches of **64×64px RGB images** with various batch sizes. For example, with an image size of **64×64px** and batch size of **20**, the generator produces images with shape **(20, 64, 64, 3)** and binary labels with shape **(20, 4)**.  

To streamline evaluation, we developed **ten functions** to test different combinations of image and batch sizes.  

We experimented with **five different architectures** by iteratively adjusting hyperparameters, including **image size, batch size, dropout rate, and convolutional layers**. The first four models had a similar structure:  

1. Several **convolutional layers**, each followed by a **max pooling layer**.  
...
# **5 - Cross-Validation**  
After using the **Hold-out Method**, we implemented the **Stratified K-Fold Cross-Validation** method to improve evaluation reliability. This approach involves splitting the training data into **K subsets**, ensuring each subset maintains the class distribution. The model is trained on **K-1 subsets**, while the remaining subset is used for validation. This process repeats until every subset has been used for evaluation, and the final results are averaged.  

The key advantage of **Cross-Validation** over the **Hold-out Method** is that it mitigates the effects of **biases or anomalies** in the dataset, leading to more stable performance metrics.  

For our case, after selecting the **top 5 models** from the Hold-out Method (with **64×64px images** and batch sizes ranging from **20 to 64**), we applied **5-Fold Cross-Validation** (**K = 5**). We limited each iteration to **15 epochs**, as models previously started overfitting around that point.  

However, we encountered a major issue when running **Cross-Validation** on the **AlexNet architecture (Model 5)**, as it consistently crashed our laptops due to high computational requirements. This highlights a key drawback of **Cross-Validation**—its significantly higher processing power demands. Given our resource limitations, this proved to be a **notable disadvantage** of the method.  
